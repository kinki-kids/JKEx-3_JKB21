...
void loop() {
  ...
  bool previous_state_right = True;//右脱線したとき


  //方向転換
  if(judgeA0 & judgeA1 & judgeA2){
    //脱線
    if(!previous_state_right){
      //右に大きく曲がる
      ...
      //左に大きく曲がる
      analogWrite(pwm_r1, 0.85*speed);
      analogWrite(pwm_r2, 0*speed);
      analogWrite(pwm_l1, 1*speed);
      analogWrite(pwm_l2, 0*speed);
    }
    Serial.println("S0");
    ...
    analogWrite(pwm_r2, 1*speed);
    analogWrite(pwm_l1, 0*speed);
    analogWrite(pwm_l2, 1*speed);
    Serial.println("S4");
  }else if(judgeA0 & judgeA1 & !judgeA2){
    //右後ろに後退する
    analogWrite(pwm_r1, 0*speed);
    analogWrite(pwm_r2, 0.85*speed);
    analogWrite(pwm_l1, 0*speed);
    analogWrite(pwm_l2, 1*speed);
    Serial.println("S5");
  }else if(judgeA0 & !judgeA1 & judgeA2){
